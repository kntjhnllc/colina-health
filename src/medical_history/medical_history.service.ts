...
export class MedicalHistoryService {
    constructor(
        @InjectRepository(MedicalHistory) private medicalHistoryRepository: Repository<MedicalHistory>,
        @InjectRepository(PatientInformation) private patientInformationRepository: Repository<PatientInformation>,
    ) {}

    async createMedicalHistoryForPatient(createMedicalHistoryForPatientInput:CreateMedicalHistoryForPatientInput): Promise<MedicalHistory> {
        const { patientId, ...medicalHistoryData } = createMedicalHistoryForPatientInput;
